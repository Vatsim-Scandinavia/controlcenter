...
FROM docker.io/library/node:21.5.0-alpine as frontend
...
####################################################################################################
# Primary container
FROM docker.io/library/php:8.2.12-apache-bullseye
...
COPY --from=mlocati/php-extension-installer:2.1.75 /usr/bin/install-php-extensions /usr/local/bin/
