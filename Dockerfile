...
FROM docker.io/library/node:21.7.2-alpine as frontend
...
####################################################################################################
# Primary container
FROM docker.io/library/php:8.3.9-apache-bullseye
...
COPY --from=mlocati/php-extension-installer:2.2.7 /usr/bin/install-php-extensions /usr/local/bin/
