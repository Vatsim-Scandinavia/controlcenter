...
FROM docker.io/library/node:24.11.0-alpine AS frontend
...
####################################################################################################
# Primary container
FROM docker.io/library/php:8.4.14-apache-bookworm
...
COPY --from=mlocati/php-extension-installer:2.9.25 /usr/bin/install-php-extensions /usr/local/bin/
