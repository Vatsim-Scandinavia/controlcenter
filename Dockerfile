...
FROM docker.io/library/node:22.8.0-alpine as frontend
...
####################################################################################################
# Primary container
FROM docker.io/library/php:8.3.10-apache-bookworm
...
COPY --from=mlocati/php-extension-installer:2.3.5 /usr/bin/install-php-extensions /usr/local/bin/
